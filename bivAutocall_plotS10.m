...
S10             = linspace(1e-5, 10000, 60);             % initial asset price S1
S20             = linspace(1e-5, 10000, 60);             % initial asset price S2
...
[S10g, S20g] = meshgrid(S10, S20);

% Vectorize grid
S10_vec = S10g(:);
S20_vec = S20g(:);

h_vec = 5e-3 * S10_vec(:)';                              % step size in finite differences for Delta

% Print parameters
fprintf('======================================================================================= \n');
fprintf('Parameters used:\n');
fprintf('  Fixed random seed                                  = %d\n', seed);
fprintf('  Initial prices S10                                 = %.2f to %.2f (step %.4f)\n', S10(1), S10(end), S10(2)-S10(1));
fprintf('  Initial prices S20                                 = %.2f to %.2f (step %.4f)\n', S20(1), S20(end), S20(2)-S20(1));
fprintf('  Number of S10 points                               = %d\n', k1);
fprintf('  Number of S20 points                               = %d\n', k2);
...
V_SMC  = bivAutocallable_SMC(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1_SMC, U2_SMC);
dV_SMC = Delta_bivAutocallable_SMC(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1_SMC, U2_SMC, h_vec);

V_OSS  = bivAutocallable_OSS(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1_OSS, U2_OSS);
dV_OSS = Delta_bivAutocallable_OSS(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1_OSS, U2_OSS, h_vec);

V_ROSS = bivAutocallable_ROSS(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1_ROSS, U2_ROSS);
dV_ROSS = Delta_bivAutocallable_ROSS(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1_ROSS, U2_ROSS, h_vec);

% Reshape back to surfaces
V_SMC  = reshape(V_SMC,  size(S10g));
dV_SMC = reshape(dV_SMC, size(S10g));
V_OSS  = reshape(V_OSS,  size(S10g));
dV_OSS = reshape(dV_OSS, size(S10g));
V_ROSS = reshape(V_ROSS, size(S10g));
dV_ROSS = reshape(dV_ROSS, size(S10g));
...
% =========================================================================
figure(1);
surf(S10g, S20g, V_SMC, 'EdgeColor', 'k', 'FaceColor', 'flat');
axis tight;
colormap turbo;
colorbar;
xlabel('S^{(1)}_{0}');
...
view(3.394960937500000e+02,32.41015625);
zlim([-1 100]);
...
% =========================================================================
figure(2);
surf(S10g, S20g, V_OSS, 'EdgeColor', 'k', 'FaceColor', 'flat');
axis tight;
colormap turbo;
colorbar;
xlabel('S^{(1)}_{0}');
...
% =========================================================================
figure(3);
surf(S10g, S20g, V_ROSS, 'EdgeColor', 'k', 'FaceColor', 'flat');
axis tight;
colormap turbo;
colorbar;
xlabel('S^{(1)}_{0}');
...
% =========================================================================
figure(4);
surf(S10g, S20g, dV_SMC, 'EdgeColor', 'k', 'FaceColor', 'flat');
axis tight;
colormap turbo;
colorbar;
xlabel('S^{(1)}_{0}');
...
zlabel('Option delta');
title('Bivariate autocallable price (SMC)');
view(11.851562500000028,19.394531249999993);
zlim([-0.003 0.1]);
...
% =========================================================================
figure(5);
surf(S10g, S20g, dV_OSS, 'EdgeColor', 'k', 'FaceColor', 'flat');
axis tight;
colormap turbo;
colorbar;
xlabel('S^{(1)}_{0}');
...
% =========================================================================
figure(6);
surf(S10g, S20g, dV_ROSS, 'EdgeColor', 'k', 'FaceColor', 'flat');
axis tight;
colormap turbo;
colorbar;
xlabel('S^{(1)}_{0}');
...
% -> Tailored for plotting against vectors S10 and S20
% =========================================================================

function V = bivAutocallable_SMC(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1, U2)
    
    S10_vec = S10_vec(:)';  % ensure S10 is a row vector
    S20_vec = S20_vec(:)';  % ensure S20 is a row vector
    k = numel(S10_vec);  % number of inital prices for S10 and S20 (same)
    ...
    S1 = repmat(S10_vec,N,1);  % paths for S1
    S2 = repmat(S20_vec,N,1);  % paths for S2
    ...
    for j=1:m
        ...
        S1(alive) = S1(alive) .* exp(drift1 + vol1 .* Y1j_mat(alive));
        S2(alive) = S2(alive) .* exp(drift2 + vol2 .* Y2j_mat(alive));
...
function dV = Delta_bivAutocallable_SMC(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1, U2, h_vec)
    % Forward finite differences OSS Monte Carlo with CRN
    S10_vec = S10_vec(:)';
    h_vec   = h_vec(:)';

    V = bivAutocallable_SMC(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1, U2);
    Vh = bivAutocallable_SMC(S10_vec+h_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1, U2);
    dV = (Vh - V)./h_vec;
end


function V = bivAutocallable_OSS(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1, U2)
    
    S10_vec = S10_vec(:)';  % ensure S10 is a row vector
    S20_vec = S20_vec(:)';  % ensure S20 is a row vector
    k = numel(S10_vec);  % number of initial S10 and S20 points (same)
    N = size(U1,1);  % number of simulations
    m = length(t)-1;  % t = (t0,t1,...,tm) = (t(1),t(2),...,t(m+1))
    dt = diff(t);  % (t(2)-t(1),...,t(m+1)-t(m))
...
function dV = Delta_bivAutocallable_OSS(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1, U2, h_vec)
    % Forward finite differences OSS Monte Carlo with CRN
    S10_vec = S10_vec(:)';
    h_vec   = h_vec(:)';
    
    V = bivAutocallable_OSS(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1, U2);
    Vh = bivAutocallable_OSS(S10_vec+h_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1, U2);
    dV = (Vh - V)./h_vec;
end


function V = bivAutocallable_ROSS(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1, U2)
...
function dV = Delta_bivAutocallable_ROSS(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1, U2, h_vec)
    % Forward finite differences OSS Monte Carlo with CRN
    S10_vec = S10_vec(:)';
    h_vec   = h_vec(:)';
    
    V = bivAutocallable_ROSS(S10_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1, U2);
    Vh = bivAutocallable_ROSS(S10_vec+h_vec, S20_vec, S1ref, S2ref, B, Q, t, r, d1, d2, sigma1, sigma2, cor, U1, U2);
    dV = (Vh - V)./h_vec;
end
